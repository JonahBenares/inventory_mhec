  ...
  function __construct(){
    ...
    public function insert_signatory(){
        ...
        for($x=1;$x<$count;$x++){
            ...
            $purchaser = $this->input->post('purchaser'.$x);
            if(!empty($requested) || !empty($noted) || !empty($inspected) || !empty($delivered) || !empty($reviewed) || !empty($received) || !empty($released) || !empty($approved) || !empty($acknowledged) || !empty($purchaser)){
                if(empty($noted)) $nt = 0;
                else $nt=1;
