     ...
     public function mrf(){
        ...
        foreach($this->super_model->select_row_where('receive_head', 'receive_id', $id) AS $us){
            $data['username'][] = array( 
                ...
                'positionpurchaser'=>$this->super_model->select_column_where('employees', 'position', 'employee_id', $us->purchaser),
                'user_id'=>$us->user_id
            );
        }
        ...
        foreach($this->super_model->select_row_where("signatories", "purchaser", "1") AS $purchaser){
            $data['purchaser_emp'][] = array( 
                'empname'=>$this->super_model->select_column_where('employees', 'employee_name', 'employee_id', $purchaser->employee_id),
                'empid'=>$purchaser->employee_id
            );
        }
